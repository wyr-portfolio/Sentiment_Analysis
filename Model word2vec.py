...
lines = list()
c = 0
for i, v in df_reviews['text'].items():
    for j in v:
        lines.append(j)
model = Word2Vec(df_reviews['text'], size=100, window=5, workers=4, min_count=1)
...
for train, test in kfold.split(x_train, y_train):
    ...
    history = model.fit(Xtrain, y_train[train], epochs=8, verbose=2)
